...
# Check parameters
if [ $# -lt 2 ]; then
  echo "Usage: play.sh game vrep_path (main_arg_1 main_arg_2 ... main_arg_n)"
  exit 1
fi
...
# Execute main script
cd $current
python3 src/main.py $rom --environment robot --play_games 10 "$@" --port $port --enable_sync $enable_sync
